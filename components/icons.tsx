export function SpanishFlagSvg() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 47.5 47.5" id="spain">
      <defs>
        <clipPath id="a">
          <path d="M0 38h38V0H0v38Z"></path>
        </clipPath>
      </defs>
      <g clip-path="url(#a)" transform="matrix(1.25 0 0 -1.25 0 47.5)">
        <path
          fill="#dd2e44"
          d="M37 10a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v18a4 4 0 0 0 4 4h28a4 4 0 0 0 4-4V10Z"
        ></path>
        <path fill="#ffcc4d" d="M37 13H1v12h36V13Z"></path>
        <path fill="#ea596e" d="M10 20v-3a3 3 0 1 1 6 0v3h-6Z"></path>
        <path fill="#f4a2b2" d="M13 18h3v3h-3v-3z"></path>
        <path fill="#dd2e44" d="M13 18h-3v3h3v-3z"></path>
        <path
          fill="#ea596e"
          d="M16 22.5c0-.829-1.343-1.5-3-1.5s-3 .671-3 1.5 1.343 1.5 3 1.5 3-.671 3-1.5"
        ></path>
        <path
          fill="#ffac33"
          d="M16 23.25c0 .414-1.343.75-3 .75s-3-.336-3-.75 1.343-.75 3-.75 3 .336 3 .75"
        ></path>
        <path fill="#99aab5" d="M8 14h1v7H8v-7zm10 0h-1v7h1v-7z"></path>
        <path
          fill="#66757f"
          d="M10 14H7v1h3v-1zm9 0h-3v1h3v-1zM9 21H8v1h1v-1zm9 0h-1v1h1v-1z"
        ></path>
      </g>
    </svg>
  );
}

export function EnglishFlagSvg() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      id="united-kingdom"
    >
      <path fill="#f0f0f0" d="M0 85.333h512V426.67H0z"></path>
      <path
        fill="#d80027"
        d="M288 85.33h-64v138.666H0v64h224v138.666h64V287.996h224v-64H288z"
      ></path>
      <path
        fill="#0052b4"
        d="M393.785 315.358 512 381.034v-65.676zM311.652 315.358 512 426.662v-31.474l-143.693-79.83zM458.634 426.662l-146.982-81.664v81.664z"
      ></path>
      <path
        fill="#f0f0f0"
        d="M311.652 315.358 512 426.662v-31.474l-143.693-79.83z"
      ></path>
      <path
        fill="#d80027"
        d="M311.652 315.358 512 426.662v-31.474l-143.693-79.83z"
      ></path>
      <path
        fill="#0052b4"
        d="M90.341 315.356 0 365.546v-50.19zM200.348 329.51v97.151H25.491z"
      ></path>
      <path
        fill="#d80027"
        d="M143.693 315.358 0 395.188v31.474l200.348-111.304z"
      ></path>
      <path
        fill="#0052b4"
        d="M118.215 196.634 0 130.958v65.676zM200.348 196.634 0 85.33v31.474l143.693 79.83zM53.366 85.33l146.982 81.664V85.33z"
      ></path>
      <path
        fill="#f0f0f0"
        d="M200.348 196.634 0 85.33v31.474l143.693 79.83z"
      ></path>
      <path
        fill="#d80027"
        d="M200.348 196.634 0 85.33v31.474l143.693 79.83z"
      ></path>
      <path
        fill="#0052b4"
        d="M421.659 196.636 512 146.446v50.19zM311.652 182.482V85.331h174.857z"
      ></path>
      <path
        fill="#d80027"
        d="M368.307 196.634 512 116.804V85.33L311.652 196.634z"
      ></path>
    </svg>
  );
}

export function FrenchFlagSvg() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" id="france">
      <path fill="#f0f0f0" d="M0 85.331h512v341.337H0z"></path>
      <path fill="#0052b4" d="M0 85.331h170.663v341.337H0z"></path>
      <path fill="#d80027" d="M341.337 85.331H512v341.337H341.337z"></path>
    </svg>
  );
}
